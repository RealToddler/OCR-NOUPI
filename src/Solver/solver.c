...
// function that solves the grid, loaded a gGrid struct
cCoords solver(char *word, gGrid *grid) {
  ...
  for (int y = 0; y < grid->rows; y++) {
    for (int x = 0; x < grid->cols; x++) {
      ...
      result = check_horizontal(grid->cols, grid->grid, word, first_coords,
                                word_size);
      ...
      result =
          check_vertical(grid->rows, grid->grid, word, first_coords, word_size);
      if (result.t2.x != -1 && result.t2.y != -1) {
        return result;
      }
