...
/*
 All theses functions are params that we pass in our apply_canny function,
 depending on what we are searching for in the given image
*/
  ...
  } else if (mod == 2) // height
  {
    ...
    for (int i = 0; i < numBoxes; i++) {
      ...
      if (boxes[i].width > (box.width - (box.width / 2)) &&
          ...
          ((boxes[i].min_x != box.min_x) && (boxes[i].min_y != box.min_y)) &&
...
void find_grid(bBoundingBox *boxes, int num_boxes, iImage *img) {
  ...
  draw_rectangle(img, x_min, y_min, x_max, y_max, red, 1);
}
...
void find_word_lists(bBoundingBox *boxes, int num_boxes, iImage *img) {
  ...
  for (int i = 0; i < index; i++) {
    draw_rectangle(img, res2.boxes[i].min_x, res2.boxes[i].min_y,
                   res2.boxes[i].max_x, res2.boxes[i].max_y, blue, 1);
  }
}
